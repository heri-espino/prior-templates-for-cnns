...
This appendix reports the fresh-data stages added after the exploratory Stage-C decomposition. Stage D was frozen before any block 4000--4019 outcome was generated. Stage E was separately frozen before any block 5000--5049 outcome was generated and is secondary to Stage D. Complete machine-readable tables are retained under \texttt{analysis/budget\_confirmation\_001/} and \texttt{analysis/exhaustive\_robustness\_001/}.
...
\subsection{Stage-D budget contrasts}\label{app:stage-d-b}

For every architecture/ranking pair,
\[
B=\frac{\Delta F(4)+\Delta F(8)}{2}-\frac{\Delta F(1)+\Delta F(2)}{2}.
\]
The TinyCNN/contrast row is the single frozen primary test. All other rows are secondary and do not alter the primary decision.
...
\bottomrule
\end{tabular}
\caption{Stage-D budget contrasts. The first row alone is confirmatory.}
\label{tab:stage-d-b}
\end{table}
...
\subsection{Stage-E default-release replication across settings}\label{app:stage-e-default}

Stage E uses 50 fresh blocks per task/architecture/profile combination. Table~\ref{tab:stage-e-default} reports the predeclared budget contrast for default release minus constant retention under the original contrast ranking. Holm adjustment is over the five non-reference profiles within each task/architecture/ranking family.
...
\begin{tabular}{llccc}
\toprule
Task & Architecture & Mean $B$ & 95\% CI & Holm $p$ \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Stage-E default-release robustness results under contrast ranking.}
\label{tab:stage-e-default}
\end{table}
...
\caption{Stage-E budget contrasts for every non-reference profile under contrast ranking.}\label{tab:stage-e-profiles}\\
\toprule
...
Table~\ref{tab:stage-e-endpoints} gives the endpoint comparison most directly related to the main narrative. Values are means over 50 fresh blocks.
