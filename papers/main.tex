...
\]
A separate exploratory one-to-one assignment analysis prevents reuse of the same template and is reported in the appendix; it does not change the behavioral conclusions.
...
\centering
\small
\begin{tabular}{p{0.08\linewidth}p{0.45\linewidth}p{0.17\linewidth}p{0.22\linewidth}}
\toprule
Stage & Scope & Blocks & Statistical role \\
\midrule
A & 400 models, 40 epochs, 10 conditions, 2 tasks, 2 architectures & 0--9 & Four locally prospective Holm-adjusted tests; other outcomes exploratory \\
B & 200 models, 200 epochs, 5 conditions, 2 tasks, 2 architectures & 2000--2009 & Longitudinal release study; reported contrasts exploratory \\
C & 80 final Stage-B checkpoints, 3 rankings, $k\in\{1,2,4,8\}$ & same as B & Post hoc measurement robustness and decomposition \\
D & 80 newly trained models, \texttt{two\_concepts}, 2 architectures, 2 conditions, 20 blocks & 4000--4019 & Frozen prospective confirmation; one primary TinyCNN test \\
E & 1,200 newly trained models, 2 tasks, 2 architectures, 6 prior profiles, 50 blocks & 5000--5049 & Predeclared secondary robustness map \\
\bottomrule
\end{tabular}
\caption{Sequential experimental stages. Stages C, D, and E were designed after earlier outcomes were known, but D and E were frozen before their own fresh blocks were generated.}
\label{tab:stages}
\end{table}
...
Stage E completed all 1,200 planned final-checkpoint evaluations on 50 fresh blocks. Under the original contrast ranking and default release schedule, the predeclared budget statistic relative to constant retention is positive for TinyCNN on both tasks: $B=0.07189$ (95\% CI $[0.05444,0.08934]$, Holm $p=1.43\times10^{-10}$) on \texttt{single\_shape}, and $B=0.28619$ ($[0.25389,0.31850]$, Holm $p=1.95\times10^{-22}$) on \texttt{two\_concepts}. The corresponding TwoLayerCNN values are $-0.01018$ ($[-0.02290,0.00254]$, Holm $p=0.4569$) and $-0.04156$ ($[-0.06983,-0.01329]$, Holm $p=0.0144$).
...
The exploratory matching analysis on the 2,000 saved Stage-B checkpoint states shows that high retained alignment is not an artifact of many learned kernels selecting the same nearest template. At epoch 200, one-to-one assignment similarity for constant retention ranges from 0.9468 to 0.9999 across the four task/architecture settings, with essentially zero nearest-versus-assignment gap, while release means range from 0.5743 to 0.9425. This result strengthens the morphology statement only: it does not establish semantic uniqueness, human readability, or the mechanism behind the patching curves.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../analysis/kernel_similarity/results/kernels_two_concepts_TinyCNN.png}
\caption{Original templates and saved first-layer kernels for \texttt{two\_concepts}/TinyCNN, block 2000. Rows are independently reordered by maximum-weight one-to-one matching. The illustration concerns morphology, not human interpretability.}
